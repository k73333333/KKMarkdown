...
## 根目录文档与脚本
- **`README.md`**: 项目说明文档，介绍应用用途、功能与基本使用方式。
- **`PACKAGING_GUIDE.md`**: 打包说明文档，记录安装包与发布相关流程。
- **`create_junctions.ps1` / `create_junctions.dart`**: Windows 环境辅助编译脚本。用于在遇到 `ERROR_ACCESS_DENIED` 时，利用目录联接（Junction）替换默认的符号链接（Symlink），以绕过系统权限或杀毒软件的硬性拦截。
- **`战斗法则-3.5-分渠道新增数据分析.md`**: 渠道新增数据分析文档，记录 2026 年 3 月四个渠道的新增表现、趋势拆解与结论建议。
